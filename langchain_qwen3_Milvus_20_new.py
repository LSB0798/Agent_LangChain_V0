...
def main():
    ...
    while True:
        ...
        print("\n🔍 正在评判用户问题是否命中风险词...")
        geted_user_input_key_words = hit_key_words(user_input, collected_key_words)
        print(f"【风险词】筛查: {geted_user_input_key_words}")

        # 2. 生成回答（仅当问题风险非“高风险”时继续，也可选择继续，根据需求）
        # 这里无论风险如何都生成回答，但会在输出中保留问题风险信息
        print("\n🤖 生成回答中...")
        answer, think = qa_system.answer_question(user_input)
        print(f"\n【模型思考】\n{think}")
        print(f"\n【模型回答】\n{answer}")
        ...
        print("\n🔍 正在评判模型回答是否命中风险词...")
        geted_ans_key_words = hit_key_words(answer, collected_key_words)
        print(f"【风险词】筛查: {geted_ans_key_words}")
        ...
        print(f"问题风险: {query_risk_level}, 关键词筛查: {geted_user_input_key_words}")
        print(f"\n模型回答: {answer}")
        print(f"回答风险: {answer_risk_level}, 关键词筛查: {geted_ans_key_words}")
        print("="*50)
