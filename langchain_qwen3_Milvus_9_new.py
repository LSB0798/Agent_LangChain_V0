"""
based on langchain_qwen3_Milvus_6_new.py
切换了OPAI-API
"""
import os
import torch
...
class Config:
    ...
    # 新增大文件处理配置
    LARGE_FILE_THRESHOLD = 100 * 1024 * 1024  # 100MB以上视为大文件
    LARGE_FILE_CHUNK_SIZE = 20 * 1024 * 1024  # 大文件分块大小（20MB）
    LARGE_FILE_USE_MMAP = True  # 启用内存映射处理大文件
    AUTO_DETECT_ENCODING = True  # 自动检测文件编码

class MmapTextLoader(BaseLoader):
    """使用内存映射处理超大文本文件的加载器 - 修复版"""
    
    def __init__(self, file_path: str, encoding: str = "utf-8", chunk_size: int = 20 * 1024 * 1024):
        self.file_path = file_path
        self.encoding = encoding
        self.chunk_size = chunk_size
    
    def load(self) -> List[Document]:
        """使用内存映射分块读取大文件 - 修复UTF-8解码问题"""
        documents = []
        try:
            file_size = os.path.getsize(self.file_path)
            print(f"使用内存映射处理大文件: {self.file_path} ({file_size/1024/1024:.2f} MB)")
            
            # 检测文件编码
            detected_encoding = self._detect_encoding()
            if detected_encoding:
                self.encoding = detected_encoding
                print(f"检测到文件编码: {self.encoding}")
            
            with open(self.file_path, 'rb') as f:  # 使用二进制模式打开
                # 使用内存映射
                with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                    ...
                    while position < file_size:
                        # 计算当前块大小
                        current_chunk_size = min(self.chunk_size, file_size - position)
                        
                        # 确保不会截断UTF-8字符
                        safe_chunk_size = self._find_safe_cutoff(mm, position, current_chunk_size)
                        ...
                        # 读取块内容
                        chunk_bytes = mm[position:position + safe_chunk_size]
                        
                        # 多种方式尝试解码
                        chunk_content = self._safe_decode(chunk_bytes)
                        
                        if not chunk_content.strip():
                            # 跳过空内容
                            position += safe_chunk_size
                            continue
                        
                        # 寻找自然断点（在句子或段落边界分割）
                        if position + safe_chunk_size < file_size:
                            last_break = self._find_natural_break(chunk_content)
                            if last_break > 0 and last_break > len(chunk_content) * 0.8:
                                # 调整内容到断点
                                chunk_content = chunk_content[:last_break]
                                safe_chunk_size = len(chunk_content.encode(self.encoding))
                        ...
                        if chunk_num % 10 == 0:
                            ...
                            print(f"文件处理进度: {progress:.1f}% ({chunk_num} 个块)")
                    
                    print(f"大文件处理完成: 共分割为 {chunk_num} 个块")
                        
        except Exception as e:
            print(f"内存映射读取文件时出错: {e}")
            # 尝试使用备用方法
        ...
        try:
            ...
            with open(self.file_path, 'rb') as f:
                ...
                if confidence > 0.7:
                    ...
                    print(f"编码检测置信度低 ({confidence:.2f})，使用默认编码: utf-8")
                    return 'utf-8'
        except ImportError:
            print("未安装chardet库，使用默认编码: utf-8")
            return 'utf-8'
        except Exception as e:
            print(f"编码检测失败: {e}，使用默认编码: utf-8")
            return 'utf-8'
    
    def _find_safe_cutoff(self, mm: mmap.mmap, position: int, chunk_size: int) -> int:
        """找到安全的切割点，避免截断UTF-8字符"""
        if position + chunk_size >= len(mm):
            return len(mm) - position
        
        # 从块末尾向前查找，找到UTF-8字符边界
        for i in range(min(4, chunk_size)):  # UTF-8字符最多4字节
            test_size = chunk_size - i
            if position + test_size > len(mm):
                continue
                
            test_bytes = mm[position:position + test_size]
            try:
                # 尝试解码，如果成功则这是安全边界
                test_bytes.decode(self.encoding, errors='strict')
                return test_size
            except UnicodeDecodeError:
                continue
        
        # 如果找不到安全边界，返回原始大小（将使用错误忽略）
        return chunk_size
    
    def _safe_decode(self, chunk_bytes: bytes) -> str:
        """安全解码字节内容"""
        # 方法1: 严格解码
        try:
            return chunk_bytes.decode(self.encoding, errors='strict')
        except UnicodeDecodeError:
            pass
        
        # 方法2: 忽略错误
        try:
            return chunk_bytes.decode(self.encoding, errors='ignore')
        except UnicodeDecodeError:
            pass
        
        # 方法3: 替换错误字符
        try:
            return chunk_bytes.decode(self.encoding, errors='replace')
        except UnicodeDecodeError:
            pass
        ...
        for enc in encodings:
            if enc == self.encoding:
                continue
            try:
                return chunk_bytes.decode(enc, errors='ignore')
            except UnicodeDecodeError:
                continue
        
        # 最后手段: 使用latin-1（不会失败）
        return chunk_bytes.decode('latin-1', errors='ignore')
    
    def _fallback_load(self) -> List[Document]:
        """备用加载方法（当内存映射失败时使用）"""
        print("内存映射失败，使用传统文件读取方式...")
        documents = []
        try:
            ...
            if len(content) > self.chunk_size:
                    ...
                    if chunk_content.strip():
                        documents.append(Document(
                            ...
                            metadata={
                                ...
                                "chunk_number": chunk_num,
                                "byte_offset": i,
                                "byte_length": len(chunk_content.encode(self.encoding)),
                                "encoding": self.encoding,
                                "type": "fallback_chunk"
                            }
                        ))
                ...
                documents.append(Document(
                    ...
                    metadata={"source": self.file_path, "type": "fallback"}
                ))
                
            print(f"传统方式加载完成: {len(documents)} 个文档块")
    ...
    def _find_natural_break(self, text: str) -> int:
        ...
        if last_paragraph > len(text) * 0.7:
        ...
        if comma_pos > len(text) * 0.9:
            return comma_pos + 1
        
        space_pos = text.rfind(' ')
        if space_pos > len(text) * 0.9:
            return space_pos + 1
...
class ConversationMemory:
    ...
    def add_conversation_turn(self, query: str, response: str, context: List[str] = None):
        """添加对话轮次到记忆 - 增强版，处理大文件上下文"""
        # 如果上下文包含大文件内容，进行简化处理
        simplified_context = []
        if context:
            for doc in context:
                if len(doc) > 500:  # 长内容简化
                    simplified_context.append(doc[:200] + "..." + doc[-200:] if len(doc) > 400 else doc)
                else:
                    simplified_context.append(doc)
        
        turn = {
            "timestamp": datetime.now().isoformat(),
            "query": query,
            "response": response,
            "context": simplified_context or [],
            "embedding": None
        }
        ...
        try:
            # 对于大文件上下文，使用简化版生成嵌入
            embedding_text = query + " " + response
            if simplified_context:
                embedding_text += " " + " ".join([ctx[:100] for ctx in simplified_context])
                
            turn["embedding"] = self.embeddings.embed_query(embedding_text)
        except Exception as e:
            print(f"生成记忆嵌入时出错: {e}")
...
class EnhancedRAGSystem:
    ...
    def _get_text_loader_for_file(self, file_path: str):
        """根据文件大小选择合适的文本加载器"""
        ...
        # 大文件使用专用加载器
        print(f"检测到大文件: {file_path} ({file_size/1024/1024:.2f}MB)，使用内存映射加载器")
        return MmapTextLoader(
            file_path, 
            encoding='utf-8', 
    ...
    def _load_documents(self):
        ...
        for filename in os.listdir(self.config.DOCUMENTS_DIR):
            file_path = os.path.join(self.config.DOCUMENTS_DIR, filename)
            ...
            try:
                if filename.lower().endswith('.txt'):
                    # 使用智能加载器（根据文件大小选择）
                    loader = self._get_text_loader_for_file(file_path)
                    docs = loader.load()
                    documents.extend(docs)
                    print(f"加载 {len(docs)} 个文本块来自: {filename}")
                    
                elif filename.lower().endswith('.pdf'):
                    loader = PyPDFLoader(file_path)
                    docs = loader.load()
                    documents.extend(docs)
                    print(f"加载 {len(docs)} 个PDF页面来自: {filename}")
                    
                elif filename.lower().endswith('.docx'):
                    loader = Docx2txtLoader(file_path)
                    docs = loader.load()
                    documents.extend(docs)
                    print(f"加载 {len(docs)} 个DOCX文档来自: {filename}")
    ...
    def _split_documents(self, documents):
        ...
        text_splitter = RecursiveCharacterTextSplitter(
            ...
            separators=["\n\n", "\n", "。", "，", " ", ""],
            length_function=lambda x: len(x)  # 按字符数计算长度（适合中文）
        )
        ...
        for doc in documents:
            ...
            if not isinstance(doc, Document):
                print(f"警告：跳过非Document类型数据 {type(doc)}")
                continue
                
            # 处理大文件块（已经预分割的块）
            if doc.metadata.get("type") == "mmap_txt_chunk" and len(doc.page_content) > self.config.CHUNK_SIZE * 2:
                # 对大文件块进行进一步分割
    ...
    def _load_or_create_vector_db(self, port):
        ...
        t2 = time.time()
        print('持续时间 : {}'.format(t2 - t1)) # 5188.383428096771
