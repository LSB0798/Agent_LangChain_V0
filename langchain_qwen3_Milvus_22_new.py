...
if FASTAPI_AVAILABLE:
    ...
    # 核心处理逻辑（提取为独立函数，供两个路由共用）
    async def _handle_chat(request: ChatRequest):
        # 提取用户输入（兼容多模态消息：content 可能是列表）
        user_input = None
    ...
    # 注册两个路由：标准 OpenAI 路径和前端代理路径
    app.post("/v1/chat/completions")(_handle_chat)
    app.post("/chat/completions")(_handle_chat)
